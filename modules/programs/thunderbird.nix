...
let
  ...
  # TODO: rename -> getAccountsForProfile?
      ...
      ++ lib.flip mapAttrsToList cfg.profiles (
        name: profile: {
          ...
          "${thunderbirdProfilesPath}/${name}/user.js" =
            let
              emailAccounts = getEmailAccountsForProfile name enabledEmailAccountsWithId;
              # TODO: REname to calendarAccounts
              calendars = getEmailAccountsForProfile name enabledCalendarAccountsWithId;
            ...
            in
            {
              text = mkUserJs (builtins.foldl' (a: b: a // b) { } (
                ...
                ++ (map toThunderbirdCalendar calendars)
              )) profile.extraConfig;
            };
        ...
        let
          emailAccountsWithFilters = (
            filter (a: a.thunderbird.messageFilters != [ ]) (
              getEmailAccountsForProfile name enabledEmailAccountsWithId
            )
          );
        in
